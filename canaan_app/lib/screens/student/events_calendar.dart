...
/// Student Dashboard → Quick Links → Events & Calendar.
///
/// Read-only: students see ONLY events relevant to them — published
/// events with audience Everyone|Students whose section is All Sections
/// or contains the student's own section (e.g. a Junior + Senior quiz
/// is hidden from Sub Junior students). Realtime delivery.
class StudentEventsCalendarPage extends StatefulWidget {
  final String studentName;
  final String? section;
  const StudentEventsCalendarPage({
    super.key,
    this.studentName = '',
    this.section,
  });
  ...
  @override
  State<StudentEventsCalendarPage> createState() =>
      _StudentEventsCalendarPageState();
}

class _StudentEventsCalendarPageState
    extends State<StudentEventsCalendarPage> {
  ...
  Future<void> _fetchEvents({bool silent = false}) async {
    ...
    try {
      ...
      final items = List<Map<String, dynamic>>.from(rows)
          .where((e) => EventService.visibleTo(
                e,
                role: 'student',
