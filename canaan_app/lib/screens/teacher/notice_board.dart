...
/// Teacher Dashboard → Quick Links → Notice Board.
///
/// Read-only. Teachers receive Teachers + All notices (filtered at the
/// query level — never fetch-all-then-hide). Realtime delivery with
/// NEW indicators cleared via the server-side `read_by` array.
class TeacherNoticeBoardPage extends StatefulWidget {
  final String teacherId;
  final String teacherName;
  const TeacherNoticeBoardPage({
    super.key,
    this.teacherId = '',
    this.teacherName = '',
  });
  ...
  @override
  State<TeacherNoticeBoardPage> createState() => _TeacherNoticeBoardPageState();
}

class _TeacherNoticeBoardPageState extends State<TeacherNoticeBoardPage> {
  ...
  Future<void> _fetchNotices({bool silent = false}) async {
    ...
    try {
      ...
      final rows = await _client
          ...
          .select('*')
          .inFilter('audience',
              NoticeService.visibleAudiencesFor('teacher'))
